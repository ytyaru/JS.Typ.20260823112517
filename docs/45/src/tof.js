...
//        const s = this._removeComments(Function.prototype.toString.call(v)).trim();
        ...
        return `${(!ag && !v.name ? 'Anonymous' : ag)}Function`; // AsyncもGeneratorも名前もない場合のみAnonymous。逆に名前がなくてもAsyncやGeneratorであればAnonymousにはならずAsyncFunctionなどになる。Anonymouseはあくまで通常関数の詳細として細分化する時だけに使うべき。それ以外は匿名か否かなどどうでもいい。それより重要な性質があるし、名前の有無は不要。
    }
