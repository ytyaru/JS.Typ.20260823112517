export class Tys {// Type string name
    ...
    static _obj(v, name) {
        ...
        if (!isPlain && 'Object'!==name && !isIns) return `NativeInstance<${name}>`;
        if (isIns) return `Instance<${proto.constructor.name}>`;
        // new Function() で生成されたES5以前の擬似クラスインスタンスの判定
        return this._isFunctionObject(v, proto) ? `ES5.Instance<${proto.constructor.name}>` : 'PrototypedObject';
//        return this._isFunctionObject(v, proto) ? 'FunctionInstance' : 'PrototypedObject';
//        return this._isFunctionObject(v, proto) ? 'FunctionObject' : 'PrototypedObject';
    ...
    */
    static _obj(v, name) {
        ...
        if (isIns) {
            ...
            //return `${(FnTys._isEs5Cls(ctor)) ? 'ES5.' : ''}Instance<${ctor.name}>`;
            return `${(FnTys._isEs5Cls(ctor) || (ctor.prototype && ctor.prototype.constructor === ctor && ctor !== Object)) ? 'ES5.' : ''}Instance<${ctor.name || '(Anonymous)'}>`;
        }
        */
        /*
        if (isIns) {
            const ctor = proto.constructor;
            // 匿名であってもES5疑似クラスのコンストラクタであれば ES5.Instance とする
            const ctorName = ctor.name || '(Anonymous)';
            // ES5疑似クラスか、あるいは匿名等で_isEs5Clsに通らなかったカスタムコンストラクタのインスタンス
//            if (FnTys._isEs5Cls(ctor) || (ctor.prototype && ctor.prototype.constructor === ctor && ctor !== Object)) {
            if (FnTys._isEs5Cls(ctor) || (proto !== Object.prototype && ctor !== Object)) {
                return `ES5.Instance<${ctorName}>`;
            }
            return `Instance<${ctorName}>`;
        }

        return 'PrototypedObject';
        //return this._isFunctionObject(v, proto) ? 'FunctionInstance' : 'PrototypedObject';
        */
        const isEs6Ins = this._isEs6Ins(proto);
        const isEs5Ins = this._isEs5Ins(v, proto);

        if (!isPlain && 'Object'!==name && !isEs6Ins && !isEs5Ins) return `NativeInstance<${name}>`;
        /*
        if (isEs6Ins) {
            return `Instance<${proto.constructor.name}>`;
        }
...
//        return FnTys._isEsCls(ctor) || FnTys._isEs5Cls(ctor);
        // ES6クラス、ES5クラス、あるいは明示的なコンストラクタを持つオブジェクト
        return FnTys._isEsCls(ctor) || FnTys._isEs5Cls(ctor) || (proto !== Object.prototype && ctor !== Object);
    }
    */
    static _isEs6Ins(proto) {
        const ctor = proto.constructor;
        if (typeof ctor !== 'function') return false;
        return FnTys._isEsCls(ctor);
    }
    static _isEs5Ins(v, proto) {
        const ctor = proto.constructor;
        if (typeof ctor !== 'function') return false;
        if (ctor === Object || ctor === Function) return false;
        if (FnTys._isEsCls(ctor) || FnTys._isNative(ctor, Function.prototype.toString.call(ctor))) return false;
        
        // ES5疑似クラス、または匿名ES5クラスインスタンスの要件を満たすか
        return FnTys._isEs5Cls(ctor) || (proto !== Object.prototype && proto !== Function.prototype);
    }
    static _isIns(v, proto) {
        return this._isEs6Ins(proto) || this._isEs5Ins(v, proto);
    }


    /*
    static _isFunctionObject(v, proto) {
        const ctor = proto.constructor;
        if (typeof ctor !== 'function') return false;
        if (FnTys._isEsCls(ctor) || FnTys._isEs5Cls(ctor)) return false;
        if (ctor.name && ctor.name !== 'Object') return true;
        return false;
    }
    */
    /*
    static _isFunctionObject(v, proto) {
        // プロトタイプが関数であり、かつプロトタイプ自身が constructor を持っている（またはObject.prototypeではない）場合の多くはES5以前の擬似クラス
        if (typeof proto === 'function') return true;
        // vのコンストラクタが関数で、そのprototypeがvの__proto__と一致する場合など
        if (v.constructor && typeof v.constructor === 'function' && Object.prototype !== proto) {
            // プレーンオブジェクト等でなく、独自のコンストラクタ関数から作られた証拠を検証
            const ctorStr = Function.prototype.toString.call(v.constructor);
            if (!FnTys._isEsCls(v.constructor, ctorStr)) {
                return true;
            }
        }
        return false;
...
//        if (this._isEs5Cls(v,s)) return `ES5.Class<${v.name}>`; // ES5擬似クラスの判定を追加
        if (this._isEsCls(v,s)) {
            const className = v.name || '(Anonymous)';
            return `Class<${className}>`;
        }
        if (this._isEs5Cls(v,s)) {
            const className = v.name || '(Anonymous)';
            return `ES5.Class<${className}>`;
        }
        if (this._isBound(v,s)) return `BoundFunction<${v.name.replace(/bound /,'')}>`;
        //if (this._isBuiltin(v,s)) return `BuiltinFunction<${v.name}>`;
        //if (this._isNative(v,s)) return `NativeFunction<${v.name}>`; // 関数かES5疑似クラスコンストラクタか区別不能！
        ...
        return this._isAB(v,s) ? 'AnonymousBlankFunction' : `${FnAgTys.name(v,s)}Function`;
//        return `${FnAgTys.name(v,s)}Function`;
    }
    static _isAB(v,s) {return !v.name && s.replace(/\/\*[\s\S]*?\*\/|\/\/.*$|\s/gm, '') === 'function(){}'}
    static _isEsCls(v,s) {
    ...
    static _isEs5Cls(v,s) {
        ...
        // class構文ではなく、かつプロトタイプにカスタムプロパティまたは自分自身を指すconstructorを持つものをES5擬似クラスと判定
        if (this._isEsCls(v,s) || this._isNative(v,s) || this._isArrow(v,s)) return false;
        
        const proto = v.prototype;
        if (!proto || typeof proto !== 'object') return false;
        
        // constructorが自分自身を指している、またはprototypeに独自のプロパティ（メソッド等）が追加されている場合
        const keys = Object.getOwnPropertyNames(proto);
        const hasCustomProps = keys.length > 1 || (keys.length === 1 && keys[0] !== 'constructor');
        const isCtorSelf = proto.constructor === v;
        
        return isCtorSelf && (hasCustomProps || /this\./.test(s));
    }
    */
    static _isEs5Cls(v,s) {
        if (!s) s = Function.prototype.toString.call(v);
        ...
        // prototypeに独自のプロパティ（メソッド等）が追加されているか
        const keys = Object.getOwnPropertyNames(proto);
        const hasCustomProps = keys.length > 1 || (keys.length === 1 && keys[0] !== 'constructor');
        if (hasCustomProps || /\bthis\./.test(s)) return true

        // 関数名が英大文字始まり（PascalCase）か
        const name = v.name || '';
...
//        const isN = s.includes('[native code]');
//        return isN ? `Native${(_isNativeClass(v) ? 'Class' : 'Function')}<${v.name}>` : false;
//    }
