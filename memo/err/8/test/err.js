...
describe("Err.registTree Tests", () => {
    test("dynamic exception hierarchy registration via TextTree", () => {
        const errorTreeText = `
AppError Error
	NetworkError
		TimeoutError
	...
	ValidationError
        ...
        Err.registTree(errorTreeText);

        const AppError = Err.get("AppError");
        const NetworkError = Err.get("NetworkError");
        const TimeoutError = Err.get("TimeoutError");
        const ValidationError = Err.get("ValidationError");

        expect(AppError).toBeDefined();
        expect(new NetworkError() instanceof AppError).toBe(true);
        expect(new TimeoutError() instanceof NetworkError).toBe(true);
        expect(new TimeoutError() instanceof AppError).toBe(true);
