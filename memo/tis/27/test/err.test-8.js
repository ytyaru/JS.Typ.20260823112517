...
describe("自作アサーションクラス Ass の厳格なテスト", () => {
    ...
    test("【型判定の検証】手作りの name 偽装エラー（PseudoTestError）に対する各API of挙動判定", () => {
...
実際値: Class [TotallyDifferentError] (name: "TotallyDifferentError")`);
    ...
    test("【異常系：共通】例外自体が発生しなかった場合、全APIが正しい型（TestFailureError）で落とすこと", () => {
        const noThrowFn = () => {};
    ...
    test("【異常系：共通】メッセージが不一致の場合、全APIが正しい型（TestFailureError）で落とすこと", () => {
