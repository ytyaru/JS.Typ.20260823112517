...
tys.jsを元にインタフェースを作る。`isT`,`owT`を作りたい。なるだけ重複コードなく。そのためにmkFO,mkErFOヘルパー関数を作っているのだが、うまく動作しない。これを動作するように実装してくれ。
...
お前のコードは以下が消されていたので追加した。

```
if (new.target) throw new ReferenceError('Constructors are not allowed.');
```

それと以下のエラーメッセージに`(v)`を追記した。

```
throw new TypeError(`Expected: a value that makes '${pathStr}(v)' return true.\nActual: ${Tys.name(v)}`);
```

それと以下が間違っている。`methods:{}`を削除した。
...
Received: null
      ...
      at assertThrow (/tmp/work/JS.Typ.20260823112517/docs/43/test/test-data.js:10:21)
✗ owT > d > num > () > ReferenceError [1.06ms]
```
