...
class Tyois {
    static some(v) {
        ...
        //return N.endsWith('Function') || ['PlainObject','Array'].some(n=>n===N) || ['Descriptor','Class','Instance'].some(n=>N.startsWith(n+'<'));
        //return 'Function'.split(' ').some(n=>N.ends(n)) || 'PlainObject Array'.split(' ').some(n=>n===N) || 'Description Class Instance'.split(' ').some(n=>N.starts(n+'<'))
    }
