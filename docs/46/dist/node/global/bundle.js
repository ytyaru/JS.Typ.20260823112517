(() => {
  // ../../../../typ-build-f1jb1vyvp2k/wrapped.js
  (function() {
    var module = { exports: {} };
...
Actual: ${tof(v)}`);
      ...
      const props = {};
      for (const key2 of Object.getOwnPropertyNames(fo)) {
        if (["some", "length", "name", "prototype", "caller", "arguments"].includes(key2))
          continue;
        const val = fo[key2];
        if (typeof val === "function") {
          ...
          if (subKeys.length > 0 || typeof val.some === "function") {
            const subPathStr = pathStr.replace(/\.some$/, `.${key2}.some`);
            props[key2] = mkErFO(val, subPathStr);
          } else {
            props[key2] = (v, ...args) => {
              if (val(v, ...args))
                return true;
