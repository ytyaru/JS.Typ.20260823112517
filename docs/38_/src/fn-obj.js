    ...
    static mkEr(isObj, pathStr) {
            ...
            if (typeof val === 'function') {
                ...
                if (subKeys.length > 0) {
                    ...
                    methods[key] = (v, ...args) => {
                        ...
                        const exp = typeof val.typeName === 'function' ? val.typeName(v, ...args) : (val.typeName || val.toString());
                        throw new TypeError(`Expected: '${exp}' like value.\nActual: ${Tys.name(v)}`);
                    };
                }
            }
        }
        ...
        if (typeof isObj._ === 'function') {
            methods._ = (n, v, ...args) => {
                ...
                const target = isObj[n];
                const exp = typeof target.typeName === 'function' ? target.typeName(v, ...args) : (target.typeName || target.toString());
                throw new TypeError(`Expected: '${exp}' like value.\nActual: ${Tys.name(v)}`);
            };
        }
