...
//        return 'Function';
    ...
    static _isMethod(v,s) {
        // メソッド構文は prototype を持たず、アロー関数（=>）でもないもの
        // コメントに関わらず、JSの仕様上この2つの条件だけで「メソッド」と「アロー関数」を通常関数から100%分離できます
        return !v.hasOwnProperty('prototype') && !s.includes('=>');
    }
    */
    static _isMethod(v,s) {
...
class FnAgTys {
    static name(v,s) {
        ...
        // 1. コンストラクタ名による判定（文字列に依存しないため100%確実）
        const cName = v.constructor?.name;
        if (cName === 'AsyncGeneratorFunction') return 'AsyncGenerator';
        if (cName === 'GeneratorFunction') return 'Generator';
        if (cName === 'AsyncFunction') return 'Async';

        // 2. 万が一コンストラクタ名が難読化等で取得できない場合のセーフティネット
        if (!s) s = Function.prototype.toString.call(v);
        
        // メソッド構文のジェネレータ（*m(){}）は function という単語を含まないため、
        // 単純に「*」が「=>」や「class」より前に存在するか、または「async*」などを簡易判定
        const hasAsterisk = s.includes('*');
        const hasAsync = s.includes('async');

        if (hasAsync && hasAsterisk) return 'AsyncGenerator';
        if (hasAsterisk) return 'Generator';
