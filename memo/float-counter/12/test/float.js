...
describe("SafeFloat / StepFloat / RoundFloat 網羅テスト", () => {
    ...
    describe("4. RoundFloat と各丸めメソッドのテスト", () => {
        test("floor（切り捨て）", () => {
            ...
            // floor の検証
            expect(rf.v).toBe(1.0);
            rf.raw = 1.9;
            expect(rf.raw).toBe(1.9);
            expect(rf.v).toBe(1.5);

            rf.raw = -1.9;
            expect(rf.v).toBe(-2.0);
            rf.raw = -2.1;
            expect(rf.v).toBe(-2.5);
            rf.raw = -2.4;
            expect(rf.v).toBe(-2.5);
            rf.raw = -2.6;
            expect(rf.v).toBe(-3.0);
        });
        test("trunc（切り捨て。負数無限丸め）", () => {
            const rf = new RoundFloat(1, RoundFloat.Method.trunc);
            rf.raw = 1.24; 
            expect(rf.v).toBe(1.0);
            rf.raw = 1.25; 
            expect(rf.v).toBe(1.0);
            ...
            rf.raw = -1.49; 
            expect(rf.v).toBe(-1.0);
            rf.raw = -1.4999999999999999999999; // 誤差
            expect(rf.v).toBe(-1.5); // 誤差
            expect(rf.raw).toBe(-1.4999999999999999999999);
            expect(rf.raw).toBe(-1.5); // 誤差
            rf.raw = -1.5; 
            expect(rf.v).toBe(-1.5);
            rf.raw = -1.6; 
            expect(rf.v).toBe(-1.5);
        ...
        test("round（四捨五入）", () => {
            const rf = new RoundFloat(1, RoundFloat.Method.round);
            rf.raw = 1.24; 
            expect(rf.v).toBe(1.0);
            rf.raw = 1.25; 
            expect(rf.v).toBe(1.5);
    ...
    describe("5. ファクトリ (float) の網羅的テスト", () => {
        ...
        test("float の各オーバーフロー修飾子（throw, ignore, stop, zero, reverse）が動作すること", () => {
            const tStep = float.step.throw(1, 1, 0, 1);
            expect(()=>tStep.up(5)).toThrow(RangeError);

            const iStep = float.step.ignore(1, 1, 0, 1);
            iStep.up(5);
            expect(iStep.v).toBe(1);

            const sStep = float.step.stop(1, 1, 0, 1);
            sStep.up(5);
            expect(sStep.v).toBe(1);

            const zStep = float.step.zero(1, 1, 0, 1);
            zStep.up(5);
            expect(zStep.v).toBe(0);

            const rStep = float.step.reverse(1, 1, 0, 1);
            rStep.up(5);
            expect(rStep.v).toBe(0);
        });
