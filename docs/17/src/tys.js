...
class FnTys {// クラスと関数を分け、関数を更に細分化する
    static name(v) {
        ...
        //return v.name ? `${FnAgTys.name(v,s)}Function`: 'AnonymousFunction';
        //return `${FnAgTys.name(v,s)}${v.name ? 'Anonymous' : ''}Function`;// Async/Generatorの時匿名と有名が区別されるせいでパターン爆発し面倒
        const ag = FnAgTys.name(v,s);
//        return ag ? `${ag}Function`: 'AnonymousFunction'; // Async/Generatorでも匿名ならAnonymousだけになってしまう！
