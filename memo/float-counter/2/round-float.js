...
class RoundFloat {
    ...
    constructor(e, over = RoundFloat.Overflow.throw) {// e:分解能指数(1/2**e)
        ...
        if (!Object.values(RoundFloat.Overflow).includes(over)) {
            throw new Error(`overはRoundFloat.Overflowのいずれかであるべきです。`);
        }
    ...
    get isIgnore() { return this._.over === RoundFloat.Overflow.ignore; }
    ...
    #count(isDown = false, n=1) {
        if (!Number.isSafeNumber(n)) {throw new TypeError(`nは1以上の安全な整数値であるべきです。`)}
...
const cReverse = new RoundFloat(52, RoundFloat.Overflow.reverse);
...
// --- 動作検証 ---
const c = new RoundFloat(2); // e=2, 分解能は 1/4 = 0.25
