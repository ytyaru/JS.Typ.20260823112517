...
//        if ('AnonymousBlankFunction'===N) return false;
    ...
    //static anonymousBlank(v) {return 'AnonymousBlankFunction'===Tys.name(v)} // 関数か匿名ES5擬似クラスか識別不能。だが関数として判断する。理由は二つ。function構文で生成されていることと、クラスなら空にする動機がないはずだから。関数ならば一時的に何もしない関数をセットし、他の関数にセットし直させる時まで空処理させることもあり得る。よって擬似クラスよりは関数として判定する。どちらでもない第三の曖昧型として扱うと、空関数にしたつもりなのに関数でなく別の型として認識されてしまい扱いづらくなってしまう。
