...
//    ary: { fn: v => Array.isArray(v), default: [] },
    ary: {fn:v=>Array.isArray(v), default: [], children: {// とにかく値が配列であれば内容がどんな状態であれ真を返す
...
//        filled: {fn:(v,T)=>Array.isArray(v) && 0<v.length && ('function'===typeof T ? v.every(x=>T(x)) : true), full:'Filled'}, // 配列かつ内容が一つ以上ある
        ...
        }, full:'Filled'}, // 配列かつ内容が一つ以上ある
        gen: {fn:(v,T)=>{
//            const t = Fn.getFlag(T).is;
//            if (!t) {throw new TypeError(`tis.ary.gen requires a validation function as the second argument.`)}
