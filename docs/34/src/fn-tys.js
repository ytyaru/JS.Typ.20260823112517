const getBody = v => Function.prototype.toString.call(v),
...
isEs6Cls = (v, s) => /^\s*class\b/.test(rmCmt((s || getBody(v)))),
isEs5Cls = (v, s) => isEs5ClsSub(v,(s || getBody(v)),v.prototype),
...
//getAgNm = (v,s)=>getAgNmCtor(v.constructor?.name) ? v.constructor?.name.replace(/Function$/, '') : getAgNmS(v,rmCmt(s||getBody(v)).trim()),
getAgNm = (v,s)=>isAgCtor(v.constructor?.name) ? v.constructor?.name.replace(/Function$/, '') : getAgNmS(v,rmCmt(s).trim()),
...
isAgCtor = n=>'AsyncGeneratorFunction'===n || 'GeneratorFunction'===n || 'AsyncFunction'===n,
...
getAgNmS2 = (isA,isG)=>isA && isG ? 'AsyncGenerator' : isG ? 'Generator' : isA ? 'Async' : '',
//getAgLikeNm = (v,s,ag)=>isArw(v,s) ? getArwNm(v,s,ag) : isMd(v,s,ag) ? getMdNm(v,s,ag) : getFnNm(v,s,ag),
getAgLikeNm = (v,s,ag)=>isArw(v,s) ? getArwNm(v,s,ag) : isMd(v,s,ag) ? getMdNm(v,s,ag) : getFnNm(v,ag),
